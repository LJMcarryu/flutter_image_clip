...
- 移除裁剪框外遮罩的高斯模糊：删除 `ImageClipEditorTheme.cropShadeBlurSigma` 字段（**破坏性变更**），裁剪框外仅保留 `#000000` / `0.3` alpha 的纯色暗遮罩。

# 0.11.2

- 将裁剪框外默认遮罩调整为 `#000000` / `0.3` alpha，并将高斯模糊 sigma 从 `20` 降为 `15`。
